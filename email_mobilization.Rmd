...
em_lp.nnm
```
### Compstinh the ATT results from different matching
...
```
## Exact matching
```{r Exact matching em1, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
set.seed(1)
em1_m.out.exa <- matchit(treated ~ female +
                        ...
                        replace = TRUE, 
                        method = "exact")
em1_bal_tab_gen <- bal.tab(em1_m.out.exa, thresholds = c(m = 0.01), un = TRUE)
em1_bal_tab_gen
```
```{r Love plot after exact matching em1, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
