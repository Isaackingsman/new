...
```
## Genetic matching
```{r Genetic matching em1, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
set.seed(1)
em1_m.out.gen <- matchit(treated ~ female +
                          race+
                          party+
                          congressionalDistrict+
                          historyCode2016+
                        ...
                        replace = TRUE, 
                        method = "genetic")
em1_bal_tab_gen <- bal.tab(em1_m.out.gen, thresholds = c(m = 0.01), un = TRUE)
em1_bal_tab_gen
```
```{r Love plot after genetic matching em1, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em1_lp.gen <- love.plot(
  m.out.gen,
  threshold = .1,
  binary = 'std',
  treat = em1_data1$treated,
