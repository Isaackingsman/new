...
em_b_m.out.nnm <- matchit(treated ~ female +
                        ...
                        replace = TRUE, 
                        method = "nearest")
em_b_bal_tab_nnm <- bal.tab(em_b_m.out.nnm, thresholds = c(m = 0.01), un = TRUE)
em_b_bal_tab_nnm
```
...
em_l_m.out.nnm <- matchit(treated ~ female +
                        ...
                        replace = TRUE, 
                        method = "nearest")
em_l_bal_tab_nnm <- bal.tab(em_l_m.out.nnm, thresholds = c(m = 0.01), un = TRUE)
em_l_bal_tab_nnm
```
...
### Regression estimate
Creating a basic and preliminary estimate of the treatment effect.
```{r em_w Experimental benchmark by regression,message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_cleaned <- em_cleaned %>%
  filter(race == 5)

model_em_w_1 <- lm(outcome ~ treated, data = em_w_cleaned)
summary(model_em_w_1)
```

### Covariate balance
```{r em_w Covariate balance ,message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_vars <- c("treated", "female","party", "congressionalDistrict", "historyCode2016", "general12", "age")
em_w_covs1 <- na.omit(em_w_cleaned[,em_w_vars])
bal.tab(em_w_covs1, treat = em_w_covs1$treated)
```

### Nearest neighbour matching
```{r em_w Matching preparation, message=FALSE, warning=FALSE, include=FALSE}
em_w_cem1_var <- c("treated", 
                   "female", 
                   "party", 
                   "congressionalDistrict", 
                   "historyCode2016", 
                   ...
                   "age", 
                   "outcome")
em_w_data1 <- na.omit(em_w_cleaned[,em_w_cem1_var])

em_w_X_1 <- em_w_data1 %>%
  dplyr::select(treated, female, party, congressionalDistrict,    historyCode2016, general12)
```
```{r em_w NNM, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_m.out.nnm <- matchit(treated ~ female +
                          party+
                          congressionalDistrict+
                          historyCode2016+
                          general12 + 
                        ...
                        replace = TRUE, 
                        method = "nearest")
em_w_bal_tab_nnm <- bal.tab(em_w_m.out.nnm, thresholds = c(m = 0.01), un = TRUE)
em_w_bal_tab_nnm
```

```{r em_w Love plot after NNM, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lp.nnm <- love.plot(
  em_w_m.out.nnm,
  threshold = .01,
  binary = 'std',
  treat = em_w_data1$treated,
  covs = em_w_X_1)
...
#### ATT estimate by nearest neighbor matching
```{r em_w Reproducing the dataset after NNM, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em_w_m.data.nnm <- match.data(em_w_m.out.nnm)
head(em_w_m.data.nnm)
```
```{r em_w Basic regression after NNM, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lm.nnm1 <- lm(outcome ~ treated, data = em_w_m.data.nnm)
summary(em_w_lm.nnm1)
```

### Exact matching
```{r em_w EXA, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_m.out.exa <- matchit(treated ~ female+
                          party+
                          congressionalDistrict+
                          historyCode2016+
                          general12 + 
                        ...
                        data = em_w_data1, 
                        method = "exact")
em_w_bal_tab_exa <- bal.tab(em_w_m.out.exa, thresholds = c(m = 0.01), un = TRUE)
em_w_bal_tab_exa
```
```{r em_w Love plot after EXA, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lp.exa <- love.plot(
  em_w_m.out.exa,
  threshold = .1,
  binary = 'std',
  treat = em_w_data1$treated,
...
#### ATT estimate by exact matching
```{r em_w Reproducing the dataset after EXA, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em_w_m.data.exa <- match.data(em_w_m.out.exa)
head(em_w_m.data.exa)
```
```{r em_w Basic regression after EXA, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lm.exa1 <- lm(outcome ~ treated, data = em_w_m.data.exa)
summary(em_w_lm.exa1)
```

The estimate for the ATT after using exact matching is 0.0190735, which has a p-value of less than 2e^-16. 

### Coarsened Exact Matching
```{r em_w CEM, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_m.out.cem <- matchit(treated ~ female +
                          party+
                          congressionalDistrict+
                          historyCode2016+
                          general12 + 
                          age,
                     data = em_w_data1, method = "cem")

em_w_bal_tab_cem <- bal.tab(em_w_m.out.cem, thresholds = c(m = 0.01), un = TRUE)
em_w_bal_tab_cem
```
```{r em_w Love plot after CEM, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lp.cem <- love.plot(
  em_w_m.out.cem,
  threshold = .01,
  binary = 'std',
  treat = em_w_data1$treated,
...
From the loveplot, we can observe that all of the adjusted differences is smaller than the unadjusted difference.
#### ATT by coarsened exact matching
```{r em_w Reproducing the dataset after CEM, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em_w_m.data.cem <- match.data(em_w_m.out.cem)
head(em_w_m.data.cem)
```
```{r Basic regression after CEM, message=FALSE, warning=FALSE, paged.print=TRUE}
em_w_lm.cem1 <- lm(outcome ~ treated, data = em_w_m.data.cem)
summary(em_w_lm.cem1)
```

The estimate for the ATT after using coarsened exact matching is 0.0213522, which has a p-value of less than 2e^-16. 

### Comparing the ATT results from different matching
```{r Generating em_w regression results after matching, message=FALSE, warning=FALSE}
# export regression result
stargazer(model_em_w_1, em_w_lm.nnm1, em_w_lm.exa1, em_w_lm.cem1,
          type="text",
          title="Regression results after matching em_w",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Generating em_w regression results after matching .doc, message=FALSE, warning=FALSE, include=FALSE}
# export regression result
stargazer(model_em_w_1, em_w_lm.nnm1, em_w_lm.exa1, em_w_lm.cem1,
          type="html",
          title="Regression results after matching em_w",
          out="regression_results_after_matching_em_w.doc",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
...
### Regression estimate 
Creating a basic and preliminary estimate of the treatment effect.
```{r em1 Experimental benchmark by regression of em1 ,message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r em1 Experimental benchmark by regression with control of em1 ,message=FALSE, warning=FALSE, paged.print=TRUE}
...
### Covariate balance
```{r em1 Covariate balance of em1,message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r em1 NNM, message=FALSE, warning=FALSE}
...
em1_bal_tab_nnm
```
```{r em1 Love plot after NNM , message=FALSE, warning=FALSE, paged.print=TRUE}
...
```
#### ATT estimate by nearest neighbor matching
```{r em1 Reproducing the dataset after NNM, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em1_m.data.nnm <- match.data(em1_m.out.nnm)
head(em1_m.data.nnm)
```
```{r em1 Basic regression after NNM, message=FALSE, warning=FALSE, paged.print=TRUE}
...
### Exact matching
```{r em1 Exact matching, message=FALSE, warning=FALSE, paged.print=TRUE}
...
em1_bal_tab_exa
```
```{r em1 Love plot after exact matching, message=FALSE, warning=FALSE, paged.print=TRUE}
...
#### ATT estimate by exact matching
```{r em1 Reproducing the dataset after EXA , message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
...
em1_bal_tab_cem
```
```{r em1 Love plot after CEM,message=FALSE, warning=FALSE, paged.print=TRUE}
...
#### ATT estimate by CEM
```{r em1 Reproducing the dataset after CEM, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
em1_m.data.cem <- match.data(em1_m.out.cem)
head(em1_m.data.cem)
```
```{r em1 Basic regression after CEM, message=FALSE, warning=FALSE, paged.print=TRUE}
...
# Comparing the ATT results of nearest neighbor matching
```{r Comparing the em ATT results of nearest neighbor matching, message=FALSE, warning=FALSE}
# export regression result
stargazer(model_em_1, em_lm.nnm1, model_em1_1, em1_lm.nnm1,  
          type="text",
          title="Regression results em NNM",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Generating em NNM regression .doc, message=FALSE, warning=FALSE, include=FALSE}
# export regression result
stargazer(model_em_1, em_lm.nnm1, model_em1_1, em1_lm.nnm1, 
          type="html",
          title="Regression results em NNM",
          out="Regression_results_NNM.doc",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Comparing the vi ATT results of nearest neighbor matching 1, message=FALSE, warning=FALSE}
# export regression result
stargazer(model_vi_1, vi_lm.nnm1, model_vi1_1, vi1_lm.nnm1,  
          type="text",
          title="Regression results vi NNM 1",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Comparing the vi ATT results of nearest neighbor matching 2, message=FALSE, warning=FALSE}
# export regression result
stargazer(model_vi2_1, vi2_lm.nnm1,  
          type="text",
          title="Regression results vi NNM 2",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Generating vi NNM regression .doc 1, message=FALSE, warning=FALSE, include=FALSE}
# export regression result
stargazer(model_vi_1, vi_lm.nnm1, model_vi1_1, vi1_lm.nnm1, 
          type="html",
          title="Regression results vi NNM 1",
          out="Regression_results_vi_NNM_1.doc",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
```{r Generating vi NNM regression .doc 2, message=FALSE, warning=FALSE, include=FALSE}
# export regression result
stargazer(model_vi2_1, vi2_lm.nnm1, 
          type="html",
          title="Regression results vi NNM 2",
          out="Regression_results_vi_NNM_2.doc",
          intercept.bottom = F,
          intercept.top = T,
          digits=4)
```
...
# To-do
balance between sample size and covariates
