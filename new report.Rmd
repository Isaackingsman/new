...
#### ATT by NNM
```{r vi Reproducing the dataset after NNM}
...
### Exact matching
```{r vi exact matching, message=FALSE, warning=FALSE}
...
```{r vi Love plot of exact matching}
vi_lp.exa <- love.plot(
  vi_m.out.exa,
  threshold = .01,
  binary = 'std',
...
#### ATT by exact
```{r vi Reproducing the dataset after exact}
...
#### ATT by CEM
```{r vi Reproducing the dataset after CEM}
...
```{r vi Creating a subset of control and treatment 1 only}
...
#### ATT by NNM
```{r vi1 Reproducing the dataset after NNM}
...
### Exact matching
```{r vi1 exact matching, message=FALSE, warning=FALSE}
...
```{r vi1 Love plot of exact matching}
...
#### ATT by exact
```{r vi1 Reproducing the dataset after exact}
vi1_m.data.exa <- match.data(vi1_m.out.exa)
head(vi1_m.data.exa)
```

#### exact regression 
```{r vi1 Basic regression after exact}
...
#### ATT by CEM
```{r vi1 Reproducing the dataset after CEM}
...
#### ATT by NNM
```{r vi2 Reproducing the dataset after NNM}
...
```{r vi2 Love plot of exact matching}
...
#### ATT by exact
```{r vi2 Reproducing the dataset after exact}
vi2_m.data.exa <- match.data(vi2_m.out.exa)
head(vi2_m.data.exa)
```

#### exact regression 
```{r vi2 Basic regression after exact}
...
#### ATT by CEM
```{r vi2 Reproducing the dataset after CEM}
